...
# 트랜스크립트 추출만 한 번 해두고, 프롬프트 튜닝하며 반복
python auto_cut.py input.mp4 --dry-run
python auto_cut.py input.mp4 --cache --dry-run   # 두 번째부터 캐시 사용
```
...
| `--llm-model` | `claude-sonnet-4-6` | 하이라이트 선정용 모델 |
...
- Claude Sonnet은 1시간 트랜스크립트(~10k 토큰) 기준 $0.05 미만
